...
// bills from sales.
	...
	int main()
	{
		char yes;
		system("cls");
		cout << " FAIRWAY GROCERY Inventory Management System program\n";
	...
	void displaystock()
	{
		...
		for (productid = 0; productid < 9; productid++)
		{
			file << productid << "\t\t\t" << products[productid] << "\t\t\t" << productsnum[productid] << "\t\t\t" << subproductsnum[productid] << "\t\t\t Rs:" << price[productid] << endl;
		}
		cout << "\nWriting to file...\n";
